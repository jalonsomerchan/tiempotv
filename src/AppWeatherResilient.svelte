<script lang="ts">
  ...
  type Loc = { id?: number; name: string; latitude: number | string; longitude: number | string; country?: string; admin1?: string; timezone?: string; country_code?: string }
  ...
  const storageKey = 'tiempotv:last-config:resilient'
  const caceres: Loc = { name: 'Cáceres', latitude: 39.4765, longitude: -6.3722, country: 'España', admin1: 'Extremadura', timezone: 'Europe/Madrid', country_code: 'ES' }
  const images = [
  ...
  const themes = ['ticker', 'dashboard', 'minimal', 'cards', 'glass', 'poster', 'lower-left', 'split', 'cinema', 'vertical']
  ...
  $: locations = unique([config.location, ...config.otherLocations])
  ...
  $: activeWeather = activeSlide ? getWeather(activeSlide.location) : null
  ...
  onMount(() => {
    ...
    if (cleanPath.startsWith('/canal') && encoded) {
      config = decodeConfig(encoded) ?? config
      viewMode = 'channel'
    } else {
  ...
  function buildSlides(): Slide[] {
    ...
    if (config.otherLocations.length) for (const section of other) {
  ...
  function getWeather(location: Loc) {
    ...
    const wantedLat = Number(location.latitude), wantedLon = Number(location.longitude)
    for (const [key, value] of Object.entries(weatherByKey)) {
      const [lat, lon] = key.split(',').map(Number)
      if (Number.isFinite(lat) && Math.abs(lat - wantedLat) < 0.08 && Math.abs(lon - wantedLon) < 0.08) return value
    }
    return null
  }
  function unique(items: Loc[]) { const map = new Map<string, Loc>(); for (const item of items.filter(Boolean).map(normalizeLoc)) map.set(keyOf(item), item); return [...map.values()] }
  function normalizeLoc(location: Loc): Loc { return { ...location, latitude: Number(location.latitude), longitude: Number(location.longitude), timezone: location.timezone || 'auto' } }
  function normalizeConfig(value: Partial<Config>): Config {
    ...
    const sections = defaultSections.map((section) => ({ ...structuredClone(section), ...(oldByType.get(section.type) ?? {}) }))
    return { ...structuredClone(defaultConfig), ...value, location: value.location ? normalizeLoc(value.location) : caceres, otherLocations: (value.otherLocations ?? []).map(normalizeLoc), otherMunicipalityMode: value.otherMunicipalityMode ?? 'grouped', showTimerRing: value.showTimerRing ?? false, sections }
  }
  function persist() { try { localStorage.setItem(storageKey, JSON.stringify(config)) } catch { /* noop */ } }
  function loadConfig() { try { const raw = localStorage.getItem(storageKey); return raw ? normalizeConfig(JSON.parse(raw)) : null } catch { return null } }
  function resetConfig() { config = structuredClone(defaultConfig); weatherByKey = {}; activeSlideIndex = 0; timerKey += 1; void refreshWeather(config) }
  ...
  function decodeConfig(value: string) { try { const normalized = value.replaceAll('-', '+').replaceAll('_', '/'); const padded = normalized + '='.repeat((4 - normalized.length % 4) % 4); return normalizeConfig(JSON.parse(decodeURIComponent(escape(atob(padded))))) } catch { return null } }
  ...
  async function refreshWeather(value: Config = config) {
    ...
    const results = await Promise.allSettled(unique([value.location, ...value.otherLocations]).map(async (location) => ({ location, weather: await fetchWeather(location) })))
    if (id !== requestId) return
    for (const result of results) if (result.status === 'fulfilled') for (const key of altKeys(result.value.location)) next[key] = result.value.weather
    weatherByKey = next
    loading = false
  ...
  function parseWeather(data: any): Weather {
    ...
    const days: Day[] = (daily.time ?? []).map((date: string, index: number) => { const precipitation = n(daily.precipitation_sum?.[index]); return { date, max: Math.round(n(daily.temperature_2m_max?.[index])), min: Math.round(n(daily.temperature_2m_min?.[index])), code: Math.round(n(daily.weather_code?.[index])), precipitation, rainProbability: precipitation > 0 ? Math.min(100, Math.round(35 + precipitation * 15)) : 0, sunrise: daily.sunrise?.[index], sunset: daily.sunset?.[index], uv: n(daily.uv_index_max?.[index]), wind: Math.round(n(daily.wind_speed_10m_max?.[index], n(current.wind_speed_10m))) } })
    const hourly = data.hourly ?? {}
    const hours: Hour[] = (hourly.time ?? []).map((time: string, index: number) => { const precipitation = n(hourly.precipitation?.[index]); return { time, temperature: Math.round(n(hourly.temperature_2m?.[index])), code: Math.round(n(hourly.weather_code?.[index])), precipitation, rainProbability: precipitation > 0 ? Math.min(100, Math.round(40 + precipitation * 20)) : 0, wind: Math.round(n(hourly.wind_speed_10m?.[index], n(current.wind_speed_10m))) } }).filter((hour: Hour) => new Date(hour.time).getTime() >= now - 3600000)
  ...
  async function searchMunicipalities(term: string, target: 'main' | 'other') { const clean = term.trim(); if (clean.length < 2) { target === 'main' ? searchResults = [] : otherSearchResults = []; return } const url = new URL('https://geocoding-api.open-meteo.com/v1/search'); url.searchParams.set('name', clean); url.searchParams.set('count', '10'); url.searchParams.set('language', 'es'); url.searchParams.set('format', 'json'); const response = await fetch(url); const data = await response.json(); const results = (data.results ?? []).map(normalizeLoc).sort((a: Loc, b: Loc) => Number((b.country_code === 'ES') || (b.country === 'España')) - Number((a.country_code === 'ES') || (a.country === 'España'))); target === 'main' ? searchResults = results : otherSearchResults = results }
  ...
  function onMainSearchBlur() { if (searchResults[0] && searchTerm.trim().toLowerCase() !== config.location.name.toLowerCase()) selectLocation(searchResults[0]) }
  function selectLocation(location: Loc) { const selected = normalizeLoc(location); config = { ...config, location: selected, channelName: config.channelName.replace(config.location.name, selected.name) }; searchTerm = selected.name; searchResults = []; weatherByKey = {}; activeSlideIndex = 0; void refreshWeather(config) }
  function addOtherLocation(location: Loc) { const selected = normalizeLoc(location); if (!config.otherLocations.some((item) => keyOf(item) === keyOf(selected))) config = { ...config, otherLocations: [...config.otherLocations, selected] }; otherSearchTerm = ''; otherSearchResults = []; void refreshWeather(config) }
  function removeOtherLocation(location: Loc) { config = { ...config, otherLocations: config.otherLocations.filter((item) => keyOf(item) !== keyOf(location)) }; void refreshWeather(config) }
  ...
  function goToSlide(index: number) { activeSlideIndex = index; timerKey += 1; startRotation() }
  function handleKeydown(event: KeyboardEvent) { if (viewMode !== 'channel' || slides.length < 2) return; if (['ArrowRight', 'ArrowDown', 'PageDown', ' ', 'Enter'].includes(event.key)) { event.preventDefault(); nextSlide() } if (['ArrowLeft', 'ArrowUp', 'PageUp', 'Backspace'].includes(event.key)) { event.preventDefault(); previousSlide() } if (event.key === 'Home') goToSlide(0); if (event.key === 'End') goToSlide(slides.length - 1) }
  function moveSection(index: number, direction: -1 | 1) { const to = index + direction; if (to < 0 || to >= config.sections.length) return; const sections = [...config.sections]; const [item] = sections.splice(index, 1); sections.splice(to, 0, item); config = { ...config, sections } }
  function onSectionDragStart(event: DragEvent, id: string) { draggedSectionId = id; event.dataTransfer?.setData('text/plain', id) }
  function onSectionDrop(event: DragEvent, targetId: string) { event.preventDefault(); const sourceId = event.dataTransfer?.getData('text/plain') || draggedSectionId; const from = config.sections.findIndex((s) => s.id === sourceId); const to = config.sections.findIndex((s) => s.id === targetId); if (from < 0 || to < 0 || from === to) return; const sections = [...config.sections]; const [item] = sections.splice(from, 1); sections.splice(to, 0, item); config = { ...config, sections }; draggedSectionId = '' }
  function toggleSection(section: Section) { config = { ...config, sections: config.sections.map((item) => item.id === section.id ? { ...item, enabled: !item.enabled } : item) } }
