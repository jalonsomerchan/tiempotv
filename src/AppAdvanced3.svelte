<script lang="ts">
  ...
  function buildSlides(): Slide[] {
    ...
    for (const section of otherSections) {
      if (config.otherMunicipalityMode === 'individual') {
        for (const location of config.otherLocations) result.push({ key: `${section.id}-${locationKey(location)}`, section, location, title: `${section.title}: ${location.name}`, mode: 'other-single' })
      } else {
  ...
  function selectLocation(location: LocationResult) { config.location = location; searchTerm = `${location.name}${location.admin1 ? `, ${location.admin1}` : ''}`; searchResults = []; activeSlideIndex = 0; void refreshWeather() }
  function addOtherLocation(location: LocationResult) { if (!config.otherLocations.some((item) => item.name === location.name && item.latitude === location.latitude)) config.otherLocations = [...config.otherLocations, location]; otherSearchTerm = ''; otherSearchResults = []; activeSlideIndex = 0; void refreshWeather() }
  function removeOtherLocation(location: LocationResult) { config.otherLocations = config.otherLocations.filter((item) => item !== location); activeSlideIndex = 0; void refreshWeather() }
  ...
  function onSectionDrop(event: DragEvent, targetSectionId: string) { event.preventDefault(); const sourceId = event.dataTransfer?.getData('text/plain') || draggedSectionId; if (!sourceId || sourceId === targetSectionId) return; const sections = [...config.sections]; const sourceIndex = sections.findIndex((section) => section.id === sourceId); const targetIndex = sections.findIndex((section) => section.id === targetSectionId); if (sourceIndex < 0 || targetIndex < 0) return; const [section] = sections.splice(sourceIndex, 1); sections.splice(targetIndex, 0, section); config.sections = sections; activeSlideIndex = 0; draggedSectionId = '' }
  function moveSection(index: number, direction: -1 | 1) { const nextIndex = index + direction; if (nextIndex < 0 || nextIndex >= config.sections.length) return; const sections = [...config.sections]; const [item] = sections.splice(index, 1); sections.splice(nextIndex, 0, item); config.sections = sections; activeSlideIndex = 0 }
  function toggleSection(section: SectionConfig) { section.enabled = !section.enabled; config.sections = [...config.sections]; activeSlideIndex = 0 }
  function toggleMetric(metric: string) { const section = config.sections.find((item) => item.type === 'interest'); if (!section) return; const metrics = new Set(section.metrics ?? []); if (metrics.has(metric)) metrics.delete(metric); else metrics.add(metric); section.metrics = [...metrics]; config.sections = [...config.sections] }
