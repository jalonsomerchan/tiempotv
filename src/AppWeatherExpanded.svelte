<script lang="ts">
  ...
  type WeatherDay = { date: string; max: number; min: number; code: number; precipitation: number; sunrise: string; sunset: string; uv: number; rainProbability: number; wind: number }
  type WeatherHour = { time: string; temperature: number; code: number; rainProbability: number; precipitation: number; wind: number }
  type Weather = { current: { temperature: number; apparent: number; humidity: number; wind: number; code: number }; daily: WeatherDay[]; past: WeatherDay[]; hourly: WeatherHour[] }
  type Slide = { key: string; section: Section; location: LocationResult; title: string; mode: SlideMode }

  const storageKey = 'tiempotv:last-config:expanded'
  ...
  const defaultSections: Section[] = [
    ...
    { id: 'rain', type: 'rain', title: 'Probabilidad de lluvia', enabled: false, days: 5 },
  ...
  const knownSections: SectionType[] = ['current', 'forecast', 'future-extremes', 'past-extremes', 'interest', 'message', 'other-current', 'other-forecast', 'hourly', 'rain', 'wind', 'sun', 'summary']
  ...
  function buildSlides(): Slide[] {
    ...
    if (config.locationMode === 'multi') {
      for (const location of allLocations) for (const section of baseSections) result.push({ key: `${keyOf(location)}-${section.id}`, section, location, title: `${section.title} · ${location.name}`, mode: 'normal' })
      return result
    }
    ...
    for (const section of otherSections) {
      if (!config.otherLocations.length) continue
      if (config.otherMunicipalityMode === 'individual') {
        for (const location of config.otherLocations) result.push({ key: `${section.id}-${keyOf(location)}`, section, location, title: `${section.title}: ${location.name}`, mode: 'other-single' })
      } else {
  ...
  function handleKeydown(event: KeyboardEvent) {
    if (viewMode !== 'channel' || slides.length < 2) return
    const target = event.target as HTMLElement | null
    if (target && ['INPUT', 'TEXTAREA', 'SELECT', 'BUTTON', 'A'].includes(target.tagName)) return
    if (['ArrowRight', 'ArrowDown', 'PageDown', ' ', 'Enter'].includes(event.key)) { event.preventDefault(); nextSlide() }
    if (['ArrowLeft', 'ArrowUp', 'PageUp', 'Backspace'].includes(event.key)) { event.preventDefault(); previousSlide() }
  ...
  function uniqueLocations(items: LocationResult[]) { const map = new Map<string, LocationResult>(); for (const item of items) map.set(keyOf(item), item); return [...map.values()] }
  function normalizeLocation(location: LocationResult): LocationResult { return { ...location, latitude: Number(location.latitude), longitude: Number(location.longitude), timezone: location.timezone || 'auto' } }
  function migrateSections(sections: Section[] | undefined): Section[] {
    const byType = new Map((sections ?? []).filter((section) => knownSections.includes(section.type as SectionType)).map((section) => [section.type, section]))
    return defaultSections.map((section) => ({ ...structuredClone(section), ...(byType.get(section.type) ?? {}) }))
  }
  function normalizeConfig(value: Partial<Config>): Config {
    return {
      ...
      location: value.location ? normalizeLocation(value.location) : caceres,
      otherLocations: (value.otherLocations ?? []).map(normalizeLocation),
      sections: migrateSections(value.sections),
    }
  }
  ...
  async function fetchWeather(location: LocationResult): Promise<Weather> {
    ...
    url.searchParams.set('daily', 'weather_code,temperature_2m_max,temperature_2m_min,precipitation_sum,precipitation_probability_max,sunrise,sunset,uv_index_max,wind_speed_10m_max')
    url.searchParams.set('hourly', 'temperature_2m,weather_code,precipitation_probability,precipitation,wind_speed_10m')
    ...
    })).filter((hour: WeatherHour) => new Date(hour.time).getTime() >= now - 60 * 60 * 1000)
  ...
  async function searchMunicipalities(term: string, target: 'main' | 'other' = 'main') {
    ...
    url.searchParams.set('name', clean); url.searchParams.set('count', '8'); url.searchParams.set('language', 'es'); url.searchParams.set('format', 'json')
    try {
      const response = await fetch(url)
      const data = await response.json()
  ...
  function selectLocation(location: LocationResult) { const selected = normalizeLocation(location); config = { ...config, location: selected, channelName: config.channelName.replace(config.location.name, selected.name) }; searchTerm = selected.name; searchResults = []; activeSlideIndex = 0; timerRingKey += 1; weatherByKey = {}; void refreshWeather(config) }
  function addOtherLocation(location: LocationResult) { const selected = normalizeLocation(location); if (config.otherLocations.some((item) => keyOf(item) === keyOf(selected))) return; config = { ...config, otherLocations: [...config.otherLocations, selected] }; otherSearchTerm = ''; otherSearchResults = []; activeSlideIndex = 0; timerRingKey += 1; void refreshWeather(config) }
  function removeOtherLocation(location: LocationResult) { config = { ...config, otherLocations: config.otherLocations.filter((item) => keyOf(item) !== keyOf(location)) }; timerRingKey += 1; void refreshWeather(config) }
