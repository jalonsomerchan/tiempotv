<script lang="ts">
  ...
  const storageKey = 'tiempotv:last-config:wmo'
  ...
  function normalizeConfig(value: Partial<Config>): Config {
    return { ...structuredClone(defaultConfig), ...value, iconStyle: value.iconStyle === 'emoji' ? 'line' : (value.iconStyle ?? 'line'), location: value.location ? normalizeLocation(value.location) : caceres, otherLocations: (value.otherLocations ?? []).map(normalizeLocation), sections: value.sections?.length ? value.sections.filter((section) => !section.type.startsWith('other-')) as Section[] : structuredClone(defaultSections) }
  }
...
{:else}
  ...
  <details class="config-group" open><summary><span>2. Apariencia</span><small>{config.theme}</small></summary><div class="config-group-body"><div class="theme-picker">{#each themes as theme}<button class:theme-selected={config.theme === theme.id} type="button" on:click={() => (config.theme = theme.id)}><strong>{theme.name}</strong></button>{/each}</div><div class="form-grid compact-colors"><label><span class="label">Fondo</span><input class="input color-input" type="color" bind:value={config.backgroundColor} /></label><label><span class="label">Texto</span><input class="input color-input" type="color" bind:value={config.textColor} /></label><label><span class="label">Recuadros</span><input class="input color-input" type="color" bind:value={config.cardColor} /></label><label><span class="label">Estilo de icono WMO</span><select class="select" bind:value={config.iconStyle}><option value="line">Línea</option><option value="solid">Sólido</option></select></label></div><label class="switch-row slim"><input type="checkbox" bind:checked={config.randomImages} /><span><strong>Imágenes aleatorias de fondo</strong></span></label></div></details>
