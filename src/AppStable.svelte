<script lang="ts">
  ...
  type Weather = { current: { temperature: number; apparent: number; humidity: number; wind: number; code: number; isDay: number }; daily: WeatherDay[]; past: WeatherDay[] }
  type Slide = { key: string; title: string; section: Section; location: LocationResult; mode: 'normal' | 'other-grouped' | 'other-single' }

  const storageKey = 'tiempotv:last-config:v3'
  ...
  $: slides = buildSlides(config, channelLocations, enabledSections)
  ...
  function getWeather(location: LocationResult) {
    const direct = weatherByKey.get(weatherKey(location))
    if (direct) return direct

    for (const [key, value] of weatherByKey.entries()) {
      const [lat, lon] = key.split(',').map(Number)
      if (Math.abs(lat - Number(location.latitude)) < 0.02 && Math.abs(lon - Number(location.longitude)) < 0.02) return value
    }

    return null
  }

  function getLocations(value: Config) {
    const map = new Map<string, LocationResult>()
    for (const location of [value.location, ...value.otherLocations]) {
      if (location) map.set(weatherKey(location), location)
    }
    return [...map.values()]
  }

  function buildSlides(value: Config, currentLocations: LocationResult[], sections: Section[]) {
    if (!value.location) return []
    const base = sections.filter((section) => !section.type.startsWith('other-'))
    const other = sections.filter((section) => section.type.startsWith('other-'))
    const result: Slide[] = []

    if (value.locationMode === 'multi') {
      for (const location of currentLocations) {
        for (const section of base) {
          result.push({ key: `${weatherKey(location)}-${section.id}`, section, location, title: `${section.title} · ${location.name}`, mode: 'normal' })
        }
      }
      return result
    }
    ...
    for (const section of other) {
      if (value.otherMunicipalityMode === 'individual') {
        for (const location of value.otherLocations) {
          result.push({ key: `${section.id}-${weatherKey(location)}`, section, location, title: `${section.title}: ${location.name}`, mode: 'other-single' })
        }
      } else {
  ...
  async function refreshWeather(value: Config = config) {
    ...
    const requestedLocations = getLocations(value)
    ...
    for (const item of settled) {
      if (item.status === 'fulfilled') {
        nextMap.set(weatherKey(item.value.location), item.value.weather)
      } else {
        failed.push('un municipio')
      }
    }
    ...
    else if (failed.length) errorMessage = ''
  }
  ...
  function selectLocation(location: LocationResult) {
    const next = { ...config, location: { ...location, latitude: Number(location.latitude), longitude: Number(location.longitude) } }
    config = next
  ...
  function toggleSection(section: Section) {
    config = { ...config, sections: config.sections.map((item) => item.id === section.id ? { ...item, enabled: !item.enabled } : item) }
    activeSlideIndex = 0
  }

  function updateSection(section: Section, patch: Partial<Section>) {
    config = { ...config, sections: config.sections.map((item) => item.id === section.id ? { ...item, ...patch } : item) }
  }

  function toggleMetric(section: Section, metric: string) {
    const metrics = new Set(section.metrics ?? [])
    if (metrics.has(metric)) metrics.delete(metric)
    else metrics.add(metric)
    updateSection(section, { metrics: [...metrics] })
  }
