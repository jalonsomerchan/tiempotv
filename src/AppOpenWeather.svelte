<script lang="ts">
  ...
  type Section = { id: string; type: SectionType | string; title: string; enabled: boolean; days?: number; message?: string; metrics?: string[] }
  type Config = {
    channelName: string
  ...
  const storageKey = 'tiempotv:last-config:openweather'
  ...
  const themes: { id: Theme; name: string }[] = [
    { id: 'ticker', name: 'CNN weather' },
    { id: 'dashboard', name: 'Weather TV' },
    { id: 'minimal', name: 'Minimal' },
    { id: 'cards', name: 'Panel público' },
    { id: 'glass', name: 'Cristal' },
    { id: 'poster', name: 'Póster' },
    { id: 'lower-left', name: 'Rótulo inferior' },
    { id: 'split', name: 'Split' },
    { id: 'cinema', name: 'Cine' },
  ...
  $: enabledSections = config.sections.filter((section) => section.enabled && isKnownSection(section.type))
  ...
  function isKnownSection(type: string): type is SectionType {
    return ['current', 'forecast', 'future-extremes', 'past-extremes', 'interest', 'message'].includes(type)
  }
  ...
  function normalizeConfig(value: Partial<Config>): Config {
    return {
      ...
      sections: value.sections?.length ? value.sections.filter((section) => isKnownSection(section.type)) : structuredClone(defaultSections),
    }
  }
  ...
  async function fetchWeather(location: LocationResult): Promise<Weather> {
    ...
    return { current: { temperature: Math.round(data.current.temperature_2m), apparent: Math.round(data.current.apparent_temperature), humidity: Math.round(data.current.relative_humidity_2m), wind: Math.round(data.current.wind_speed_10m), code: data.current.weather_code }, daily: days.filter((day) => day.date >= today), past: days.filter((day) => day.date < today).reverse() }
  }
